...
<div class="row ">
    ...
    <div class="col-lg-12"  style="display-flex">
          <form  style="display: contents;" action="{{ route('receptionist.expance_report.filter_by_date') }}" method="post">
               @csrf
        ...
        <form  style="display: contents;" action="{{ route('receptionist.expance_report.filter_by_user') }}" method="post">
              @csrf
             <select style="margin-left: 20px; background-color: #ffff;"  id="expense_category" name="id" required="">
                ...
                <option value="{{$user->id }}"  <?php echo  ($user_id == $user->id)?'selected':''; ?> >{{$user->booking_number }} || <?php echo isset($user->guest_details->name) ? $user->guest_details->name : ''?> </option>
     @endforeach
               ...
               <a href="{{ route('receptionist.expance_report.report') }}" class="btn btn-primary mx-5" style="background:gray;border:gray" >Reset</a>
...
<tbody>
    ...
    @if($user_id == 0 && $item->particulars != 'test')
    ...
    <tr>
        ...
        @endif
         <td style="text-align:left;" data-label="@lang('payment')">{{$item->guest_details->payment_mode}}</td> 
...
<td style="text-align:left;" data-label="@lang('Particulars')">
    ...
    @elseif($user_id == $item->id && $item->particulars != 'test')
     <tr>
        <td data-label="@lang('S.N.')">{{ $table2Data->firstItem() + $loop->index-1 }}</td>
        @if(isset($item->debit) && isset($item->credit))
